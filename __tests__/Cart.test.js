...
describe("Testing the class Cart", () => {
  ...
  describe("Testing the method addItem", () => {
    ...
    test("with argument count equal 0 or <0", () => {
      expect(() => new Cart().addItem({ name: "house", price: 10 }, 0)).toThrow(
        "Please enter the correct quantity of goods!",
      );
      expect(() => new Cart().addItem({ name: "car", price: 3 }, -5)).toThrow(
        "Please enter the correct quantity of goods!",
      );
    });
    test("with zero and negative price", () => {
      ...
      expect(priceZero).toEqual({
        items: [{ item: { name: "phone", price: 0 }, count: 2 }],
      });
      expect(() =>
        new Cart().addItem({ name: "house", price: -10 }, 2),
      ).toThrow("A product cannot have a negative price!");
    });
    test("with invalid name", () => {
      const invalidName = new Cart();
      const invalidNameFunc = () =>
        invalidName.addItem({ name: 123, price: 0 }, 2);
  ...
  describe("Testing the method getCount", () => {
    ...
    test("with zero count", () => {
      expect(() =>
        new Cart().addItem({ name: "house", price: 10 }, 0).getCount(),
      ).toThrow("Please enter the correct quantity of goods!");
    });
    test("with negative count", () => {
      expect(() =>
        new Cart().addItem({ name: "house", price: 10 }, -2).getCount(),
      ).toThrow("Please enter the correct quantity of goods!");
    });
  });
});
