...
describe("Testing the function makeUser", () => {
  test("with correctly values", () => {
    const user1 = makeUser({
      id: 1,
      friends: [makeUser({ id: 1 }), makeUser({ id: 2 })],
    });
  ...
  test("with invalid values", () => {
    expect(() => makeUser({ id: null, friends: "" })).toThrow(
      "id must be a number or undefined!",
    );
    expect(() => makeUser({ id: 1, friends: "" })).toThrow(
      "friends must be an array!",
    );
    expect(() => makeUser({ id: "string", friends: [] })).toThrow(
      "id must be a number or undefined!",
    );
    expect(() => makeUser({ id: 1, friends: {} })).toThrow(
      "friends must be an array!",
    );
    expect(() => makeUser({ id: 1, friends: 123 })).toThrow(
      "friends must be an array!",
    );
    expect(() => makeUser({ id: 1, friends: null })).toThrow(
      "friends must be an array!",
    );
    expect(() =>
      makeUser({ id: 1, friends: [{ id: 2 }, "invalid friend"] }),
    ).toThrow("Each friend must be an object with an 'id' property!");
  });
  test("with empty values", () => {
    ...
    expect(makeUser({ friends: [] })).toEqual({
      id: undefined,
      friends: [],
      getFriends: expect.any(Function),
    });
