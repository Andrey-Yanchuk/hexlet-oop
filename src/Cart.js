...
export class Cart {
    ...
    addItem(item, count = 1) {
        if (count <= 0) throw new Error("Please enter the correct quantity of goods!");
        if (!item.name || typeof item.name !== 'string') throw new Error("Please enter a valid name!");
        if (typeof item.price !== 'number' || item.price < 0) throw new Error("A product cannot have a negative price!");
